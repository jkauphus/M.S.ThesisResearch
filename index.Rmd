...
## Multivariate Analysis of Cottonmouth habitat compared to general available habitat.
...
kable(PermDisp, caption = "Table 1: Results of the multivariate statistal tests PERMANOVA/PERMDISP based on the Euclidean distance comparing cottonmouth habitat selected and general habitat available to the cottonmouth within the study area.")%>%
...
## Multivariate Analysis Comparing Cottonmouth Groups
...
pairs<-pairwise.perm.manova(vegdist(s[,2:24]), s$Type, nperm = 999, test = "Hotelling-Lawley")
...
cv_table<-cv_table[c(3,4,30),]
RandomForest_Model<-c("Maximum", "Mean", "Minimum")
```
```{r Calculating AUC}
#Minimal Model AUC
pred1<-predict(c10, test, type = "prob")
pred1<-pred1[,2]
...
#Mean Model AUC
pred1<-predict(a4, test, type = "prob")
pred1<-pred1[,2]
pred1<-prediction(pred1, test$Type)
...
#Mean Model AUC
pred1<-predict(a3, test, type = "prob")
pred1<-pred1[,2]
pred1<-prediction(pred1, test$Type)
...
Habitat_Variables <- c("Substrate Temp","Surface Temp", "Ambient Temp","Cover Length", "Cover Width", "Canopy Closure", "Rock Cover", "Leaf Cover", "Vegetative Cover", "Fallen Log Cover", "Woody Stem Diameter","Woody Stem Height", "Cover Height", "Distance To Cover","Log Diameter", "Distance To Water", "Water Depth", "Percent Land", "Percent Water", "Percent Woody Debris", "Percent Debris", "Percent Floating Vegetation", "Percent Bare Soil")
D<-cbind(Habitat_Variables,D[,1])
D<-data.frame(D)
names(D)<-c("Habitat Variables", "Mean Gini Index")
D$`Mean Gini Index`<-as.numeric(D$`Mean Gini Index`)


G<-ggplot(D, aes( x = reorder(`Habitat Variables`, `Mean Gini Index`), y=`Mean Gini Index`))+
  geom_col(width = .4)+
  ylim(0,25)+
  coord_flip()+
...
w<-partialPlot(a3, train, Dist..To.Water, "SNAKE", plot = F)
x<-partialPlot(a3, train, Depth, "SNAKE", plot = F)
y<-partialPlot(a3, train, X.Bare.soil, "SNAKE", plot = F)
z<-partialPlot(a3, train, Surface.Temp, "SNAKE", plot =F)
...
z1<-ggplot(data = z, aes(x,y))+
  ...
  ylab("Relative probability")+
  geom_text(aes(37, 0.3), label = paste("D"), col = "black")+
  theme_bw()
```
