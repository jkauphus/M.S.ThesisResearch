...
kable(PermDisp, digits = 4, caption = "Table 1: Results of the multivariate statistal tests PERMANOVA/PERMDISP based on the Euclidean distance comparing cottonmouth habitat selected and general habitat available to the cottonmouth within the study area.")%>%
...
#Kable
kable(PermDisp, digits = 3, caption = "Table 2: Results of the multivariate statistal tests PERMANOVA/PERMDISP based on the Euclidean distance comparing habitat selected differnces among the cottonmouth groups within the study area.")%>%
  kable_styling(c("striped", "bordered"))%>%
...
ggplot(PCA2, aes(x= SVL, y= Comp.1))+
  ...
  geom_text(aes(880, 5.4, label = paste("F= 3.29;P< 0.01")), color = "black")+
...
```
**Figure 2:** Linear regression of Principal Component scores of PC-1 comparison of habitat selection among the cottonmouth groups (Males, Females, Juveniles, and YOY) and cottonmouth snout-vent lengths. 
...
```{r Cross Validation}
p1<-predict(a1, test)
cv1<-confusionMatrix(p1, positive = "SNAKE",test$Type)
p2<-predict(a2, test)
cv2<-confusionMatrix(p2, test$Type,positive = "SNAKE")
p3<-predict(a3, test)
cv3<-confusionMatrix(p3, test$Type, positive = "SNAKE")
p4<-predict(a4, test)
cv4<-confusionMatrix(p4, test$Type, positive = "SNAKE")
p5<-predict(a5, test)
cv5<-confusionMatrix(p5, test$Type, positive = "SNAKE")
p6<-predict(a6, test)
cv6<-confusionMatrix(p6, test$Type, positive = "SNAKE")
p7<-predict(a7, test)
cv7<-confusionMatrix(p7, test$Type, positive = "SNAKE")
p8<-predict(a8, test)
cv8<-confusionMatrix(p8, test$Type, positive = "SNAKE")
p9<-predict(a9, test)
cv9<-confusionMatrix(p9, test$Type, positive = "SNAKE")
p10<-predict(a10, test)
cv10<-confusionMatrix(p10, test$Type, positive = "SNAKE")
p11<-predict(b1, test)
cv11<-confusionMatrix(p11, test$Type, positive = "SNAKE")
p12<-predict(b2, test)
cv12<-confusionMatrix(p12, test$Type, positive = "SNAKE")
p13<-predict(b3, test)
cv13<-confusionMatrix(p13, test$Type, positive = "SNAKE")
p14<-predict(b4, test)
cv14<-confusionMatrix(p14, test$Type, positive = "SNAKE")
p15<-predict(b5, test)
cv15<-confusionMatrix(p15, test$Type, positive = "SNAKE")
p16<-predict(b6, test)
cv16<-confusionMatrix(p16, test$Type, positive = "SNAKE")
p17<-predict(b7, test)
cv17<-confusionMatrix(p17, test$Type, positive = "SNAKE")
p18<-predict(b8, test)
cv18<-confusionMatrix(p18, test$Type, positive = "SNAKE")
p19<-predict(b9, test)
cv19<-confusionMatrix(p19, test$Type, positive = "SNAKE")
p20<-predict(b10, test)
cv20<-confusionMatrix(p20, test$Type, positive = "SNAKE")
p21<-predict(c1, test)
cv21<-confusionMatrix(p21, test$Type, positive = "SNAKE")
p22<-predict(c2, test)
cv22<-confusionMatrix(p22, test$Type, positive = "SNAKE")
p23<-predict(c3, test)
cv23<-confusionMatrix(p23, test$Type, positive = "SNAKE")
p24<-predict(c4, test)
cv24<-confusionMatrix(p24, test$Type, positive = "SNAKE")
p25<-predict(c5, test)
cv25<-confusionMatrix(p25, test$Type, positive = "SNAKE")
p26<-predict(c6, test)
cv26<-confusionMatrix(p26, test$Type, positive = "SNAKE")
p27<-predict(c7, test)
cv27<-confusionMatrix(p27, test$Type, positive = "SNAKE")
p28<-predict(c8, test)
cv28<-confusionMatrix(p28, test$Type, positive = "SNAKE")
p29<-predict(c9, test)
cv29<-confusionMatrix(p29, test$Type, positive = "SNAKE")
p30<-predict(c10, test)
cv30<-confusionMatrix(p30, test$Type, positive = "SNAKE")
#Creating CV Table
...
Gini<-importance(c10, class = "SNAKE")
